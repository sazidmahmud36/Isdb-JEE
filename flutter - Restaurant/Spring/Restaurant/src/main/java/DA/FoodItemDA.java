...
public class FoodItemDA extends MysqlConnection{
	...
	public void addFood(FoodItem f) {
		String AddFoodStatement = "insert into food_item(foodName,foodPrice,foodQty) values(?,?,?)";
		try {
			con = DriverManager.getConnection(url,user,pass);
			ps = con.prepareStatement(AddFoodStatement);
			ps.setString(1, f.getFoodName());
			ps.setInt(2, f.getFoodPrice());
			ps.setInt(3, f.getFoodQty());
...
//			ps.setInt(1, foodId);
			...
			while(rs.next()) {
				getAllFood.add(new FoodItem(rs.getInt(1),rs.getString(2),rs.getInt(3),rs.getInt(4)));
			}
